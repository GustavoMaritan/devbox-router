...
class Controller {
    ...
    static _prepare(method, params) {
        ...
        return _routeIsValid(route) ? route : null;
    }
    ...
    static _routeIsValid(route) {
        ...
        let $route = this._validRoutes.find(x =>
            x.uri.replace(/\:\D\w+/g, 'xxx') == route.uri.replace(/\:\D\w+/g, 'xxx') &&
        ...
        this._errorRoutes.push(
            colors.red('♦ Rota não registrada \n') +
    ...
    static _urlExists(rota, ctrl) {
        let newUrl = rota.uri.replace(/\:\D\w+/g, 'xxx');
