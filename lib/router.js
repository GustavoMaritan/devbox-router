...
class Controller {
    ...
    static _routeIsValid(route) {
        ...
        const controllerName = this._controller.replace('/', '');
        if (!this._errorRoutes[controllerName])
            this._errorRoutes[controllerName] = { rotas: [] };

        this._errorRoutes[controllerName].rotas.push({
            item: { ctrl: controllerName, url: route.uri, method: route.method },
            conflito: { ctrl: $route.controller.replace('/', ''), url: $route.uri, method: $route.method }
        });
